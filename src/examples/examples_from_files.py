...
class TestSimilarityEvaluator(Evaluator[str, str]):
    ...
    def evaluate(self, evaluation_case: EvaluationData[str, str]) -> EvaluationOutput:
        ...
        return EvaluationOutput(score=score, test_pass=True if score > 0.5 else False)
...
def tools_task(query: str) -> dict:
    ...
    return {"output": str(response), "trajectory": list(response.metrics.tool_metrics.keys())}
...
if __name__ == "__main__":
    ...
    files = [
        "custom_evaluator_dataset",
        "judge_output_dataset",
        "memory_messages_dataset",
        "messages_trajectory_dataset",
        "tools_trajectory_dataset",
    ]
    tasks = [get_response, get_response, memory_messages_task, memory_messages_task, tools_task]
    for f, t in zip(files, tasks, strict=False):
